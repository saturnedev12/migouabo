...
<div class="container">
    <div class="row">
        <div class="col-xl-10 main-content pl-xl-4 pr-xl-5 col-xl-offset-1">
            <h4 id="card-group">Détail catégorie : {{ $categorie->name }}</h4>
            ...
            <div class="example">
                <div class="card-group">
                    <div class="card">
                    <img src="{{ url('users/assets/images/categorie_image/'.$categorie->photo ) }}" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h3 class="card-title">{{ $categorie->name }}</h3>
                            <p class="card-text">{{ $categorie->description }}</p>
                            <p class="card-text"><small class="text-muted">Ajouté par {{ $categorie->user->name }} le {{ $categorie->created_at->format('d/m/Y') }}</small></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="col-md-6 mt-3">
        <a href="{{ route('categories.edit', $categorie->id) }}" class="btn btn-primary btn-block btn-lg"><i data-feather="pen-tool"></i> Modifier la Catégorie</a>
    </div>
</div>
@endsection
